...
<div class="card">
  ...
  <div class="row"><span class="lbl">Temperatura</span><span id="st-temp">—</span></div>
  <div class="row"><span class="lbl">Humedad</span><span id="st-hum">—</span></div>
...
async function sync(){
  try{
    ...
    document.getElementById('st-temp').innerHTML=
      '<span style="color:#ff6b6b;font-weight:700">🌡 '+d.temp.toFixed(1)+' °C</span>';
    document.getElementById('st-hum').innerHTML=
      '<span style="color:#339af0;font-weight:700">💧 '+d.hum.toFixed(1)+' %</span>';
...
void actualizarDHT() {
  unsigned long now = millis();
  if (now - estado.ultimaDHT < 2000) return;
  estado.ultimaDHT = now;

  float t = dht.readTemperature();
  float h = dht.readHumidity();
  if (!isnan(t)) estado.temp = t;
  if (!isnan(h)) estado.hum  = h;
  ...
  oled.setTextSize(1);
  oled.setCursor(0, 0);
  oled.println("SMART LIGHTING");
  oled.drawLine(0, 10, 127, 10, SSD1306_WHITE);

  oled.setTextSize(2);
  oled.setCursor(0, 16);
  oled.print(estado.temp, 1);
  oled.println(" C");

  oled.setCursor(0, 36);
  oled.print(estado.hum, 1);
  oled.println(" %");
  ...
  bool oscuro = digitalRead(PIN_LDR) != LOW;
  bool mov    = digitalRead(PIN_PIR) == HIGH;
  oled.print(oscuro ? "OSC " : "LUZ ");
  oled.print(mov    ? "MOV " : "    ");
  oled.print(estado.ledOn ? "LED:ON" : "LED:OFF");
...
void handleStatus() {
  ...
  j += "\"r\":";        j += estado.r; j += ",";
  j += "\"g\":";        j += estado.g; j += ",";
  j += "\"temp\":";     j += estado.temp; j += ",";
  j += "\"hum\":";      j += estado.hum;  j += ",";
  j += "\"b\":";        j += estado.b; j += "}";

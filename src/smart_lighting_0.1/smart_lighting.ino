...
const char* WIFI_PASSWORD = "TU_PASSWORD";   // Contraseña de tu red WiFi
...
<div class="container">
  ...
  <div class="status-card">
    ...
    <div class="status-row">
      <span class="status-label">Movimiento</span>
  ...
  <div class="modos-grid">
    ...
    <button class="btn-modo" id="btn-dia" onclick="setModo('dia')">
      <span class="icon">☀️</span>Día
    </button>
    <button class="btn-modo" id="btn-noche" onclick="setModo('noche')">
      <span class="icon">🌙</span>Noche
    </button>
...
<script>
  ...
  let modoActivo = 'auto';
  let previewR = 255, previewG = 0, previewB = 0;

  // ── Actualizar previsualización de sliders ──
  function actualizarPreviewManual() {
    previewR = parseInt(document.getElementById('sl-r').value);
    previewG = parseInt(document.getElementById('sl-g').value);
    previewB = parseInt(document.getElementById('sl-b').value);
    document.getElementById('val-r').textContent = previewR;
    document.getElementById('val-g').textContent = previewG;
    document.getElementById('val-b').textContent = previewB;
  }
  ...
  function actualizarBotones(m) {
    ['auto','dia','noche','fiesta','off'].forEach(id => {
  ...
  async function actualizarStatus() {
    try {
      ...
      // Modo
      const modoLabels = { auto:'AUTO', dia:'DÍA', noche:'NOCHE', fiesta:'FIESTA', off:'OFF' };
      const modoClases = { auto:'tag-auto', dia:'tag-on', noche:'tag-auto', fiesta:'tag-on', off:'tag-off' };
      document.getElementById('st-modo').innerHTML =
        '<span class="tag ' + (modoClases[data.modo]||'tag-auto') + '">' + (modoLabels[data.modo]||data.modo) + '</span>';

      // LDR
      document.getElementById('st-ldr').innerHTML =
        data.oscuro
          ? '<span style="color:#ffd166">🌙 Oscuro (' + data.ldr + ')</span>'
          : '<span style="color:#ffd166">☀️ Con luz (' + data.ldr + ')</span>';

      // PIR
      document.getElementById('st-pir').innerHTML =
        data.pir
          ? '<span class="tag tag-on">🚶 Detectado</span>'
...
/** GET /modo?m=auto|dia|noche|fiesta|off → Cambia el modo */
void handleModo() {
  ...
  else if (m == "dia")    { estado.modo = MODO_DIA;    setColor(220, 230, 255); } // blanco frío
  else if (m == "noche")  { estado.modo = MODO_NOCHE;  setColor(8, 8, 50);     } // azul muy tenue
...
void handleStatus() {
  // Actualizar sensores si estamos en modo distinto a AUTO
  // (en AUTO ya los lee el loop, aquí es solo para la UI)
  if (estado.modo != MODO_AUTO) {
    estado.valorLDR      = analogRead(PIN_LDR);
    estado.estaOscuro    = (estado.valorLDR > LDR_UMBRAL_OSCURO);
    estado.hayMovimiento = digitalRead(PIN_PIR);
  }

  String modos[] = {"auto", "dia", "noche", "fiesta", "off"};

  // Construir JSON manualmente (evita dependencia de ArduinoJson en runtime)
  String json = "{";
...
void setup() {
  ...
  // LED 1
  ledcSetup(CH_R1, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcSetup(CH_G1, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcSetup(CH_B1, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcAttachPin(PIN_R1, CH_R1);
  ledcAttachPin(PIN_G1, CH_G1);
  ledcAttachPin(PIN_B1, CH_B1);

  // LED 2
  ledcSetup(CH_R2, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcSetup(CH_G2, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcSetup(CH_B2, PWM_FRECUENCIA, PWM_RESOLUCION);
  ledcAttachPin(PIN_R2, CH_R2);
  ledcAttachPin(PIN_G2, CH_G2);
  ledcAttachPin(PIN_B2, CH_B2);

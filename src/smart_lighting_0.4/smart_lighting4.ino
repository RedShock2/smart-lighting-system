...
#define LDR_UMBRAL               1000
#define TIMEOUT_MOVIMIENTO_MS   15000
...
void setColor(uint8_t r, uint8_t g, uint8_t b) {
  uint8_t br = estado.brillo;
  ledcWrite(PIN_R, r * br / 100);
  ledcWrite(PIN_G, g * br / 100);
  ledcWrite(PIN_B, b * br / 100);
...
void loopAuto() {
  ...
  bool oscuro = analogRead(PIN_LDR) < LDR_UMBRAL;
  bool mov    = digitalRead(PIN_PIR);
  if (mov) estado.ultimoMovimiento = now;

  if (oscuro && (now - estado.ultimoMovimiento < TIMEOUT_MOVIMIENTO_MS)) {
    setColor(200, 140, 60);
  } else {
    apagar();
  }
...
.on {background:rgba(46,213,115,0.15);color:var(--g);border:1px solid rgba(46,213,115,0.3)}
.off{background:rgba(255,71,87,0.15); color:var(--d);border:1px solid rgba(255,71,87,0.3)}
.au {background:rgba(0,212,255,0.15); color:var(--a);border:1px solid rgba(0,212,255,0.3)}
.bar-wrap{width:80px;height:5px;border-radius:3px;background:#1a2332;overflow:hidden}
...
.btn.active-f{border-color:var(--a2);background:rgba(123,47,255,0.1);color:var(--a2);animation:fb 1s infinite alternate}
@keyframes fb{from{border-color:var(--a2)}to{border-color:#ffd166}}
.btn-off{width:100%;background:rgba(255,71,87,0.08);border:1px solid rgba(255,71,87,0.3);border-radius:12px;padding:14px;color:var(--d);font-family:'Orbitron',monospace;font-size:12px;font-weight:700;letter-spacing:2px;cursor:pointer;transition:all .2s;margin-bottom:18px}
.btn-off:hover{background:rgba(255,71,87,0.18);transform:translateY(-1px)}
.btn-off.active{background:rgba(255,71,87,0.2)}
...
.sl-lbl{font-family:'Orbitron',monospace;font-size:11px;font-weight:700}
.sl-val{font-family:'Orbitron',monospace;font-size:11px;color:var(--m);text-align:right}
...
<div class="card">
  <div class="row"><span class="lbl">Modo</span><span id="st-modo"><span class="tag au">AUTO</span></span></div>
  <div class="row">
    <span class="lbl">Luz ambiente</span>
    <div id="st-ldr" style="display:flex;flex-direction:column;align-items:flex-end;gap:3px">—</div>
  </div>
...
<div class="panel">
  <div class="srow">
    <span class="sl-lbl" style="color:#ffd166">☀</span>
    <input type="range" id="sl-b" min="0" max="100" value="100"
           style="background:linear-gradient(90deg,#1a1a00,#ffd166)"
           oninput="document.getElementById('val-b').textContent=this.value+'%'"/>
    <span class="sl-val" id="val-b">100%</span>
  </div>
  <button class="apply" onclick="enviar('/brillo?v='+document.getElementById('sl-b').value)">▶ APLICAR</button>
</div>

<div class="sec">Color manual</div>
<div class="panel">
  <div class="srow">
    <span class="sl-lbl" style="color:#ff6b6b">R</span>
    <input type="range" id="sl-r" min="0" max="255" value="255"
           style="background:linear-gradient(90deg,#1a0000,#ff6b6b)"
           oninput="document.getElementById('val-r').textContent=this.value"/>
    <span class="sl-val" id="val-r">255</span>
  </div>
  <div class="srow">
    <span class="sl-lbl" style="color:#51cf66">G</span>
    <input type="range" id="sl-g" min="0" max="255" value="0"
           style="background:linear-gradient(90deg,#001a00,#51cf66)"
           oninput="document.getElementById('val-g').textContent=this.value"/>
    <span class="sl-val" id="val-g">0</span>
  </div>
  <div class="srow">
    <span class="sl-lbl" style="color:#339af0">B</span>
    <input type="range" id="sl-b2" min="0" max="255" value="0"
           style="background:linear-gradient(90deg,#00001a,#339af0)"
           oninput="document.getElementById('val-b2').textContent=this.value"/>
    <span class="sl-val" id="val-b2">0</span>
  </div>
...
async function aplicarColor() {
  ...
  const b=document.getElementById('sl-b2').value;
...
function marcaBotones(m) {
  ['auto','fiesta','off'].forEach(id=>{
    ...
    if(!btn) return;
    btn.className = id==='off'?'btn-off':'btn';
    if(id===m){
      if(id==='off')         btn.classList.add('active');
      else if(id==='fiesta') btn.classList.add('active-f');
...
async function sync() {
  try {
    ...
    const mc={auto:'au',fiesta:'on',off:'off'};
    document.getElementById('st-modo').innerHTML=
      '<span class="tag '+mc[d.modo]+'">'+ml[d.modo]+'</span>';
    const pct=Math.round(d.ldr/4095*100);
    const col=d.oscuro?'#ffd166':'#00d4ff';
    document.getElementById('st-ldr').innerHTML=
      '<span style="color:'+col+';font-weight:700">'+(d.oscuro?'🌙 Oscuro':'☀️ Con luz')+'</span>'+
      '<div class="bar-wrap"><div class="bar-fill" style="width:'+pct+'%;background:'+col+'"></div></div>'+
      '<span style="font-size:10px;color:#5a6478">'+d.ldr+' / 4095</span>';
    document.getElementById('st-pir').innerHTML=d.pir
      ?'<span class="tag on">🚶 DETECTADO</span>'
      :'<span class="tag off">— Sin movimiento</span>';
    document.getElementById('st-led').innerHTML=d.led
      ?'<span class="tag on">ENCENDIDO</span>'
      :'<span class="tag off">APAGADO</span>';
    ...
    document.getElementById('sl-b').value=d.brillo;
    document.getElementById('val-b').textContent=d.brillo+'%';
    marcaBotones(d.modo);
  } catch(e){}
}
sync();
...
void handleStatus() {
  ...
  bool osc = ldr < LDR_UMBRAL;
  String modos[]={"auto","fiesta","off"};
  String j="{";
  j+="\"modo\":\""+modos[estado.modo]+"\",";
  j+="\"ldr\":"+String(ldr)+",";
  j+="\"oscuro\":"; j+=(osc?"true":"false"); j+=",";
  j+="\"pir\":";    j+=(pir?"true":"false"); j+=",";
  j+="\"led\":";    j+=(estado.ledOn?"true":"false"); j+=",";
  j+="\"brillo\":"+String(estado.brillo)+",";
  j+="\"r\":"+String(estado.r)+",";
  j+="\"g\":"+String(estado.g)+",";
  j+="\"b\":"+String(estado.b)+"}";
